...
INPUT_PATH = './training_set/image_data/' #directory of MFCC nFeatures x nFrames 2-D array .npy files
TARGET_PATH = './training_set/target_data/' #directory of nCharacters 1-D array .npy files
...
####Network Parameters
nFeatures = 220 #12 MFCC coefficients + energy, and derivatives
nHidden = 128
nClasses = 63 #27 characters, plus the "blank" for CTC
...
with graph.as_default():
    ...
    forward = rnn_cell.MultiRNNCell([forwardH1] * 6, state_is_tuple=True)
    backward = rnn_cell.MultiRNNCell([backwardH1] * 6, state_is_tuple=True)
    fbH1, _, _ = bidirectional_rnn(forward, backward, inputList, dtype=tf.float32,
                                       scope='BDLSTM_H1')
    ...
    pred = ctc.ctc_beam_search_decoder(logits3d, seqLengths)
...
with tf.Session(graph=graph) as session:
    ...
    for epoch in range(nEpochs):
        ...
        for batch, batchOrigI in enumerate(batchRandIxs):
            ...
            TargetLabel = np.zeros(batchTargetShape)
            for i in range(batchTargetVals.shape[0]):
                index = batchTargetIxs[i]
                TargetLabel[index[0]][index[1]] = batchTargetVals[i]
            #print (TargetLabel.shape)
            print(batchTargetShape)
            batchTargetLabel = np.split(batchTargetVals, 4)          
            feedDict = {inputX: batchInputs, targetIxs: batchTargetIxs, targetVals: batchTargetVals,
                        targetShape: batchTargetShape, seqLengths: batchSeqLengths}
            _, l, er, lmt, pre = session.run([optimizer, loss, errorRate, logitsMaxTest, pred], feed_dict=feedDict)
            print (pre)
            '''
            predictLabel = np.zeros(pre.shape)
            for i in range(pre.values.shape[0]):
                index = pre.indices[i]
                predictLabel[index[0]][index[1]] = pre.values[i]
            print (predictLabel.shape)
            print (np.sum(np.sum(TargetLabel == predictLabel,axis = 1)))
            '''
            '''
