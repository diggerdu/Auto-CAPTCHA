...
train_data_node = tf.placeholder(data_type(), shape=(BATCH_SIZE, IMAGE_HEIGHT, IMAGE_WIDTH * 4))
train_labels_node = tf.placeholder(tf.int64, shape=(BATCH_SIZE, RNN_STEPS))
eval_data = tf.placeholder(data_type(), shape=(EVAL_BATCH_SIZE, IMAGE_HEIGHT, IMAGE_WIDTH * 4))
...
fc1_weights = tf.Variable(  # fully connected, depth 512.
    tf.truncated_normal([IMAGE_HEIGHT / 4 *IMAGE_WIDTH * 4 / 4 / RNN_STEPS, 512],
                        stddev=0.1,
                        seed=SEED,
                        dtype=data_type()))
...
def model(data, train=False):
    ...
    for output in pool_output_list:
        output = tf.reshape(output, [pool_shape[0], pool_shape[1] * pool_shape[2] * pool_shape[3] / RNN_STEPS])
        conv_net_output_list.append(tf.nn.relu(tf.matmul(output, fc1_weights) + fc1_biases))
...
outputs, states = rnn.rnn(rnn_net, cnn_output_list, dtype=tf.float32)
loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(outputs, train_labels_node))
